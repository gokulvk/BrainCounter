...
parentDirectory = substring(imageDirectory, 0, lastIndexOf(substring(imageDirectory, 0, lengthOf(imageDirectory) - 1), "\\") + 1);
if (left) {
	atlasName = atlasNum + "_left.tif";
	atlasDirectory = parentDirectory + "Atlas Left\\" + atlasName;
}
else {
	atlasName = atlasNum + "_right.tif";
	atlasDirectory = parentDirectory + "Atlas Right\\" + atlasName;
}

//rescales the atlas image correctly
open(atlasDirectory);
run("Scale...", "x=- y=- width=" + toString(width) + " height=" + toString(height) + " interpolation=Bilinear average create title=scaled.tif");
...
landmarkDirectory = parentDirectory + "Landmarks/";
...
f = File.open(parentDirectory + "File Landmarks\\" + name + "_landmarks.txt");
...
renamed = File.rename(parentDirectory + "File Landmarks\\" + name + "_landmarks.txt", parentDirectory + "File Landmarks\\" + name + "_landmarks.csv");

//runs bigwarp for user to change manually
selectWindow("scaled.tif");
run("Big Warp", "moving_image=" + fullName + " target_image=scaled.tif");
selectWindow("scaled.tif");
close();
